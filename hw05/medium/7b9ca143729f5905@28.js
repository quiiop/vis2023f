...
function _medium1(d3,output,drag,invalidation)
{
   // 指定圖表的尺寸。
  const width = 1500;
  const height = 1000;
  ...
  // 添加節點外框
  const circleRadius = 50; // 調整圓圈半徑大小
  node.append("circle")
      .attr("r", circleRadius)
...
function _medium2(d3,output,drag,invalidation)
{
   // 指定圖表的尺寸。
  const width = 1500;
  const height = 1000;
  ...
  node.append("circle")
      .attr("r", circleRadius)
      .attr("fill", "gray") // 內部填充顏色
      .attr("stroke", d => colorScale(d.depth)) // 外框顏色根據節點深度
      .attr("stroke-width", 3);

  //設定圖片大小
  const size_offset = 1.5;//控制內圖片大小
...
function _medium3(d3,output,drag,invalidation)
{
   // 指定圖表的尺寸。
  const width = 1500;
  const height = 1000;
  ...
  // 添加節點外框
  const circleRadius = 40; // 調整圓圈半徑大小
  node.append("circle")
      .attr("r", circleRadius)
      .attr("fill", "red") // 內部填充顏色
      .attr("stroke", d => colorScale(d.depth)) // 外框顏色根據節點深度
      .attr("stroke-width", 3);
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["output (1).json", {url: new URL("./files/a34be24b36c7a17baf57b36f5f008044c1a1a2fbda93dd225e8cb0db2c996a3de5ba8749c3a62e07cbabcfa01d704c9555506e60a2bc97f4a88a680ef4964c53.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("output")).define("output", ["FileAttachment"], _output);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("medium1")).define("medium1", ["d3","output","drag","invalidation"], _medium1);
  main.variable(observer("medium2")).define("medium2", ["d3","output","drag","invalidation"], _medium2);
  main.variable(observer("medium3")).define("medium3", ["d3","output","drag","invalidation"], _medium3);
  return main;
}
