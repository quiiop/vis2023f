...
FileAttachment("output (2)@3.json").json()
)}

function _strong(d3,strong_,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 2000
    ;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(strong_);
...
function _strong_(FileAttachment){return(
FileAttachment("output (2)@3.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["output (2)@3.json", {url: new URL("./files/4d68b0f4a41df2d35367f4fd1095364f9fec8fd4f3fe94ccac6af422d6c2b1da4744f3c740d35344652c45f4209e9a11cce75b1c3875b46f3ef8e1265e9dbf73.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("output")).define("output", ["FileAttachment"], _output);
  main.variable(observer("strong")).define("strong", ["d3","strong_","drag","url_list","invalidation"], _strong);
  main.variable(observer("strong_")).define("strong_", ["FileAttachment"], _strong_);
